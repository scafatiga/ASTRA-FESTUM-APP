document.addEventListener('DOMContentLoaded', () => {
    ...
    // Función para añadir una línea de Gasto con su desplegable de Punto de Venta
    function agregarGastoFila() {
        const div = document.createElement('div');
        div.className = 'flex items-center space-x-2';
        div.innerHTML = `
            <input type="text" placeholder="Concepto / Descripción" class="gasto-desc border p-2 rounded w-5/12 text-sm" required>
            <input type="number" step="0.01" placeholder="Importe (€)" class="gasto-importe border p-2 rounded w-3/12 text-sm" required>
            <select class="gasto-pv border p-2 rounded w-3/12 text-sm">
                <option value="Alicante">Alicante</option>
                <option value="Madrid">Madrid</option>
            </select>
            <button type="button" class="bg-red-500 hover:bg-red-600 text-white px-2.5 py-2 rounded text-sm transition" onclick="this.parentElement.remove()">✕</button>
        `;
        gastosContainer.appendChild(div);
    }

    // Función para añadir una línea de Adelanto con su desplegable de Punto de Venta
    function agregarAdelantoFila() {
        const div = document.createElement('div');
        div.className = 'flex items-center space-x-2';
        div.innerHTML = `
            <input type="text" placeholder="Nombre Empleado" class="adelanto-emp border p-2 rounded w-5/12 text-sm" required>
            <input type="number" step="0.01" placeholder="Importe (€)" class="adelanto-importe border p-2 rounded w-3/12 text-sm" required>
            <select class="adelanto-pv border p-2 rounded w-3/12 text-sm">
                <option value="Alicante">Alicante</option>
                <option value="Madrid">Madrid</option>
            </select>
            <button type="button" class="bg-red-500 hover:bg-red-600 text-white px-2.5 py-2 rounded text-sm transition" onclick="this.parentElement.remove()">✕</button>
    ...
    btnAgregarGasto.addEventListener('click', agregarGastoFila);
    btnAgregarAdelanto.addEventListener('click', agregarAdelantoFila);
