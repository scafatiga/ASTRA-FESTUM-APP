document.addEventListener('DOMContentLoaded', async () => {
    ...
    inputExcel.addEventListener('change', async () => {
        ...
        try {
            const res = await fetch('/api/personal/importar-excel', { method: 'POST', body: formData });
            const resultado = await res.json();
            if (!res.ok) throw new Error(resultado.error || 'Error al importar el archivo');

            const avisoPV = resultado.sinPuntoVenta > 0 ? ` ${resultado.sinPuntoVenta} fila(s) con un Punto de Venta que no se encontró (revísalas).` : '';
            resultadoImportacion.textContent = `Importación completada: ${resultado.creados} nuevo(s), ${resultado.actualizados} actualizado(s), ${resultado.omitidos} omitido(s) (sin nombre), de ${resultado.total} filas leídas.${avisoPV}`;
